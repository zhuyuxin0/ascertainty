 ...
 * (chrome) ▸ overlays (tooltip + toast).
 *
 * Owns no state itself — the Zustand store at lib/atlas-v3/state.ts is
 * the single source of truth. Loads live counts from /atlas/models,
 * /atlas/markets, /bounties on mount and feeds them down as props
 * (those don't belong in the global state — they're cached fetches). */
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") {
        ...
        else if (s.lasso) setLasso(false);
